...
namespace LoL_Matchup_CLI_Tool.Helpers
{
    static class ParamValidator
    {
        ...
        {
            const string exMess = "Provide lane in correct format! ex.'top', 'jg', 'mid', 'adc' or 'sup'.";
        ...
        internal static bool GetUserChamps(string[] userChamps, HashSet<string> laneChampions)
        {
            ...
            foreach (var champ in userChamps)
            {
                if (!laneChampions.Any(x => x.Equals(champ, StringComparison.CurrentCultureIgnoreCase)))
                {
